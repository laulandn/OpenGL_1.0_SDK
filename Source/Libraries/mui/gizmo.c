...
muiButton	*newbed(void)
{
    muiButton	*b;

    b = (muiButton *)malloc(sizeof(muiButton));
    b->str[0] = 0;
    b->type = BED;
    return b;
}
...
{
    int temp;
    muiButton *but = (muiButton *)b->object;
...
muiTextBox *activetb = 0;

muiTextBox	*newtb(int xmin, int xmax)
{
    muiTextBox	*tb;
    static int inited = 0;
    ...
    tb = (muiTextBox *)malloc(sizeof(muiTextBox));
...
{
    muiTextBox *tb = (muiTextBox *)obj->object;
    return tb->str;
}
...
void loadtb(muiTextBox *tb, char *s)
{
    if (s == 0)
	*tb->str = 0;
    else
...
void backspacetb(muiTextBox *tb)
{
    char *s1, *s2, *stemp;
...
void inserttbchar(muiTextBox *tb, char c)
{
    char	*s1, *s2;
    int	len;
...
enum muiReturnValue textboxhandler(muiObject *obj, int event, int value, int x, int y)
{
	...
	case MUI_KEYSTROKE:
	    ...
	    else if (value == '\b') { backspacetb((muiTextBox *)obj->object); }
	    else inserttbchar((muiTextBox *)obj->object, (char)value);
	    break;
    }
    x = y;  /* for lint's sake */
