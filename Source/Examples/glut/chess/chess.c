...
extern GLfloat CX, CY, CX2, CY2, CZ2;
...
void do_pieces(void)
{
	...
	do_piece(piece&15, CX, CY, &stuntm, &cyclem, WIT);
	glPopMatrix();
    }

    if ((piece2&16) == WIT && piece2 > 0)
	do_piece(piece2&15, CX2, CY, &stunt2, &cycle2, WIT);
	...
	glPushMatrix();
	glTranslatef(0.0, 0.2, 0.0);
	do_piece(piece&15, CX, CY, &stuntm, &cyclem, ZWART);
	glPopMatrix();
    }
