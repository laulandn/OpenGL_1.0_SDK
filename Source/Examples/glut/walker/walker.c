...
int MakeLoadAndSaveMenus(void)
{
  ...
  CSetNames[csetnum] = strdup("bound");
  glutAddMenuEntry(CSetNames[csetnum], csetnum);
  csetnum++;
  
  CSetNames[csetnum] = strdup("dunk");
  glutAddMenuEntry(CSetNames[csetnum], csetnum);
  csetnum++;
  
  CSetNames[csetnum] = strdup("Impossible");
  glutAddMenuEntry(CSetNames[csetnum], csetnum);
  csetnum++;
  
  CSetNames[csetnum] = strdup("kick");
  glutAddMenuEntry(CSetNames[csetnum], csetnum);
  csetnum++;
  
  CSetNames[csetnum] = strdup("moonwalk");
  glutAddMenuEntry(CSetNames[csetnum], csetnum);
  csetnum++;
  
  CSetNames[csetnum] = strdup("MrFlex");
  glutAddMenuEntry(CSetNames[csetnum], csetnum);
  csetnum++;
  
  CSetNames[csetnum] = strdup("Ouch");
  glutAddMenuEntry(CSetNames[csetnum], csetnum);
  csetnum++;
  
  CSetNames[csetnum] = strdup("reverse_dunk");
  glutAddMenuEntry(CSetNames[csetnum], csetnum);
  csetnum++;
  
  CSetNames[csetnum] = strdup("running");
  glutAddMenuEntry(CSetNames[csetnum], csetnum);
  csetnum++;
  
  CSetNames[csetnum] = strdup("walking");
  glutAddMenuEntry(CSetNames[csetnum], csetnum);
  csetnum++;
  
  CSetNames[csetnum] = strdup("walk_backwards");
  glutAddMenuEntry(CSetNames[csetnum], csetnum);
  csetnum++;
...
void CurveMenuInit(void)
{
  ...
  glutAttachMenuName(GLUT_RIGHT_BUTTON, "Curve");
}
...
void GuyReshape(int w, int h)
{
  ...
  glLoadIdentity();
  if (ViewPerspective)
    gluPerspective(60.0, (GLdouble)w/h, 1, 10);
  else
    glOrtho(-1.2, 1.2, -1.2, 1.2, 1, 10);
  glMatrixMode(GL_MODELVIEW);
  glFlush();
  glutSwapBuffers();
}
...
void GuyMenuInit(void)
{
  ...
  //glutAttachMenu(GLUT_RIGHT_BUTTON);
  glutAttachMenuName(GLUT_RIGHT_BUTTON, "Guy");
}
